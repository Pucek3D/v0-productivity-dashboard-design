...
export function TopPrioCard({ tasks, setTasks, taskMeta, updateTaskMeta, openModal, onTaskToggle }: TopPrioCardProps) {
  ...
  const deleteTask = (sectionKey: string, taskIdx: number) => {
    setTasks(prev => {
      const n = [...prev]; const si = n.findIndex(s => s.section === sectionKey)
      if (si < 0) return prev
      const s = { ...n[si] }; s.tasks = s.tasks.filter((_, i) => i !== taskIdx); n[si] = s; return n
    })
  }
  ...
  const addTask = (sectionKey: string) => {
    setTasks(prev => {
      const n = [...prev]; const si = n.findIndex(s => s.section === sectionKey)
      if (si < 0) return prev
      const s = { ...n[si] }; s.tasks = [...s.tasks, { id: `q${Date.now()}`, text: 'New task', done: false }]; n[si] = s; return n
    })
  }
  ...
  const renameTask = (sectionKey: string, taskIdx: number, newText: string) => {
    setTasks(prev => {
      const n = [...prev]; const si = n.findIndex(s => s.section === sectionKey)
      if (si < 0) return prev
      const s = { ...n[si] }; const tl = [...s.tasks]; tl[taskIdx] = { ...tl[taskIdx], text: newText }; s.tasks = tl; n[si] = s; return n
    })
  }

  const handleDragStart = (event: DragStartEvent) => {
    for (const section of tasks) {
      const found = section.tasks.find(t => t.id === event.active.id)
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!destSection) {
      ...
      if (tasks.find(s => s.section === overKey)) {
        destSection = overKey; destIdx = tasks.find(s => s.section === overKey)!.tasks.length
      }
    }
    ...
    if (srcSection === destSection) {
      if (destIdx < 0) return
      setTasks(prev => {
        const n = [...prev]; const si = n.findIndex(s => s.section === srcSection)
        const s = { ...n[si] }; s.tasks = arrayMove([...s.tasks], srcIdx, destIdx); n[si] = s; return n
      })
    } else {
      setTasks(prev => {
        const n = [...prev]
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <div className="card-base halo-black">
        ...
        <div className="px-3 py-3">
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <Quadrant sectionKey="Home" label="HOME" labelColor="#a78bfa" tasks={homeTasks?.tasks || []}
        ...
        <div className="px-3 py-3">
          <div className="grid grid-cols-2 gap-2.5">
            <Quadrant sectionKey="Other Work" label="WORK" labelColor="#818cf8" tasks={otherWork?.tasks || []}
              ...
              openModal={openModal} taskMeta={taskMeta} updateTaskMeta={updateTaskMeta} />
            <Quadrant sectionKey="Other Home" label="HOME" labelColor="#a78bfa" tasks={otherHome?.tasks || []}
      ...
      <DragOverlay>
        {activeTask ? (
          <div className="bg-[#1e2438] border border-white/10 rounded-md px-2 py-1 text-[12px] text-slate-200 shadow-xl">{activeTask.text}</div>
        ) : null}
      </DragOverlay>
    </DndContext>
  )
  ...
  return (
    <div ref={setNodeRef} className="rounded-lg p-2.5 border transition-colors"
      ...
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button onClick={onAdd} className="w-4 h-4 flex items-center justify-center rounded text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors">
          <IconPlus size={11} />
        </button>
      </div>
      <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
        {tasks.map((task, ti) => (
          <SortableTask key={task.id} task={task}
            onToggle={() => onToggle(ti)} onDelete={() => onDelete(ti)}
            onOpen={() => openModal(`prio-${task.id}`, task.text)}
            onRename={(t) => onRename(ti, t)}
...
function SortableTask({ task, onToggle, onDelete, onOpen, onRename, taskMeta, updateTaskMeta }: {
  task: PrioTask; onToggle: () => void; onDelete: () => void; onOpen: () => void
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1 }}
      ...
      <EditableText value={task.text} onChange={onRename}
        className={`text-[12px] leading-[1.35] flex-1 min-w-0 truncate ${task.done ? 'text-slate-500 line-through' : 'text-slate-200'}`} />
