...
function SortableTask({ task, onToggle, onDelete, onOpen, onRename, taskMeta, updateTaskMeta, overflow = false }: any) {
  ...
  return (
    <div ref={setNodeRef} {...dndProps}
      ...
      <div onPointerDown={(e:any)=>e.stopPropagation()} onClick={overflow ? (e:any)=>e.stopPropagation() : (e:any)=>{e.stopPropagation();onToggle()}} className={`w-3.5 h-3.5 rounded-[4px] border flex-shrink-0 flex items-center justify-center transition-all mt-[2px] ${overflow?'border-slate-700 bg-white/[0.02]':`cursor-pointer ${task.done?'bg-indigo-500/30 border-indigo-400':'border-slate-600 bg-white/5 group-hover:border-slate-400'}`}`}>{task.done&&!overflow&&<span className="text-indigo-300 text-[8px] font-bold leading-none">✓</span>}</div>
