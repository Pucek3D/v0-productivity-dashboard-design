...
import { LT_GOALS, statusStyle, type Project } from '@/lib/data'
      ...
      <div className="p-2 grid grid-cols-2 gap-1.5">
        {LT_GOALS.map(goal => (
          <GoalTile
            ...
            completion={getProjectCompletion(goal)}
            isExpanded={expanded[goal.key] || false}
            onToggleExpand={() => toggleExpand(goal.key)}
          />
        ))}
      </div>
    </div>
...
function GoalTile({
  ...
  return (
    <div
      ...
      style={{ borderLeftWidth: 3, borderLeftColor: goal.color }}
    >
      ...
      <div className="flex items-center gap-1 mb-0.5">
        <div className="flex-1 h-[3px] bg-gray-100 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${completion}%`, backgroundColor: goal.color }}
          />
        </div>
      ...
      <div className="border-t border-gray-50 pt-0.5 mt-0.5">
        {visibleTasks.map((task, i) => {
          const isDone = projectDone[`${goal.key}-task-${i}`] || false
          return (
            <TaskRow
              key={i}
              text={task}
              done={isDone}
              onToggle={() => toggleProjectTask(goal.key, 'task', i)}
            />
          )
        })}
      </div>
      ...
      {hasMore && (
        <>
          {isExpanded && (
            <div className="pt-0.5">
              {hiddenTasks.map((task, i) => {
                const isDone = projectDone[`${goal.key}-task-${i + 2}`] || false
                return (
                  <TaskRow
                    key={`hidden-${i}`}
                    text={task}
                    done={isDone}
                    onToggle={() => toggleProjectTask(goal.key, 'task', i + 2)}
                  />
                )
              })}
              {goal.doneTasks.map((task, i) => {
                ...
                return (
                  <TaskRow
                    ...
                    onToggle={() => toggleProjectTask(goal.key, 'done', i)}
                  />
                )
              })}
            </div>
          ...
          >
            ...
            <span className={`text-[8.5px] text-[#10b981] transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
              ▼
            </span>
          </div>
        </>
