...
export function OtherTodoCard({ taskMeta, updateTaskMeta, openModal, starToPrio, isTaskStarred }: Props) {
  ...
  const toggleTask = (sid: string, tid: string) => setSections(prev => prev.map(s => s.id===sid ? {...s,tasks:s.tasks.map(t=>t.id===tid?{...t,done:!t.done}:t)} : s))
  const deleteTask = (sid: string, tid: string) => setSections(prev => prev.map(s => s.id===sid ? {...s,tasks:s.tasks.filter(t=>t.id!==tid)} : s))
  const addTask = (sid: string) => setSections(prev => prev.map(s => s.id===sid ? {...s,tasks:[...s.tasks,{id:`ot${Date.now()}`,text:'New task',done:false}]} : s))
  const deleteSection = (sid: string) => setSections(prev => prev.filter(s => s.id!==sid))
  const renameSection = (sid: string, name: string) => setSections(prev => prev.map(s => s.id===sid ? {...s,name} : s))
  const renameTask = (sid: string, tid: string, text: string) => setSections(prev => prev.map(s => s.id===sid ? {...s,tasks:s.tasks.map(t=>t.id===tid?{...t,text}:t)} : s))
  const addSection = () => setSections(prev => [...prev,{id:`sec-${Date.now()}`,name:'New Section',color:SECTION_COLORS[Math.floor(Math.random()*SECTION_COLORS.length)],tasks:[]}])
          ...
          {sections.map(section => (
            <div key={section.id}>
              ...
              {section.tasks.map(task => {
                const taskKey = `todo-${section.id}-${task.id}`
                const meta = taskMeta[taskKey]
                const firstSub = meta?.subtasks?.find(s => !s.done)
                return (
                  <div key={task.id}>
                    {/* Task row — CLICKABLE to open modal */}
                    <div className="flex items-center gap-1.5 py-[3px] group cursor-pointer" onClick={() => openModal(taskKey, task.text)}>
                      <div onClick={(e) => {e.stopPropagation(); toggleTask(section.id, task.id)}}
                        className={`w-3.5 h-3.5 rounded-[4px] border flex-shrink-0 flex items-center justify-center cursor-pointer ${task.done?'bg-indigo-500/30 border-indigo-400':'border-slate-600 bg-white/5'}`}>
                        {task.done&&<span className="text-indigo-300 text-[8px] font-bold leading-none">✓</span>}
                      </div>
                      <EditableText value={task.text} onChange={(t:string)=>renameTask(section.id,task.id,t)}
                      ...
                      <span className="inline-flex items-center gap-0.5 flex-shrink-0" onClick={(e:any)=>e.stopPropagation()}>
                        ...
                        {starToPrio && <button onClick={()=>starToPrio(task.text,'work')} className="icon-on-hover bg-transparent border-none cursor-pointer p-0 leading-none">
                          <IconStar size={11} className={isTaskStarred?.(task.text)?'fill-yellow-500 text-yellow-500':'text-slate-500 hover:text-amber-400'} />
                        </button>}
                        <button onClick={(e)=>{e.stopPropagation();deleteTask(section.id,task.id)}} className="icon-on-hover bg-transparent border-none cursor-pointer p-0 leading-none"><IconTrash size={11} className="text-slate-500 hover:text-rose-400" /></button>
                      </span>
                    </div>
                    {/* Meta badges */}
                    ...
                    {firstSub && <div className="flex items-center gap-1.5 pl-[22px] pb-0.5">
                      ...
                      <div className="w-3 h-3 rounded-[3px] border border-slate-600 bg-white/5 flex-shrink-0 cursor-pointer hover:border-slate-400"
                        onClick={() => { const subs=(meta?.subtasks||[]).map((s:any)=>s.id===firstSub.id?{...s,done:true}:s); updateTaskMeta(taskKey,{subtasks:subs}) }} />
