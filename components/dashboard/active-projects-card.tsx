      ...
      <div className="p-2.5">
        ...
        <div className="border-t border-white/5 pt-1 mt-1">
          ...
          {visibleCustom.length > 0 && (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onReorderCustomTasks}>
    ...
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 }} className="py-0.5 cursor-pointer select-none group" onClick={onOpen}>
      <div className="flex items-center gap-1.5">
        ...
        <span className="inline-flex items-center gap-0.5 flex-shrink-0" onClick={(e: any) => e.stopPropagation()}>
          ...
          <button className="bg-transparent border-none cursor-pointer p-0 leading-none" onClick={(e: any) => { e.stopPropagation(); onStar() }}><IconStar size={12} className={isStarred ? 'fill-yellow-500 text-yellow-500' : 'icon-on-hover text-slate-500 hover:text-amber-400'} /></button>
          {onBookmark && <button className="bg-transparent border-none cursor-pointer p-0 leading-none" onClick={(e: any) => { e.stopPropagation(); onBookmark() }} title={isBookmarked ? 'Added to Other to-dos' : 'Add to Other to-dos'}><IconBookmark size={12} className={isBookmarked ? 'fill-indigo-400 text-indigo-400' : 'icon-on-hover text-slate-500 hover:text-indigo-300'} /></button>}
