  ...
  return createPortal(<>
    ...
    <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 10001, width: 540, maxHeight: '85vh', overflowY: 'auto', background: '#0f1623', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.5)' }}>
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        {/* Checklist — subtasks with owner, deadline, star */}
        <Sec label={`Checklist${sTotal > 0 ? ` (${sDone}/${sTotal})` : ''}`}>
          ...
          {(meta.subtasks||[]).map(st => {
            const stDeadline = (st as any).deadline
            const stDeadlineInfo = stDeadline ? getDateLabel(stDeadline) : null
            return (
              <div key={st.id} style={{ padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {starSubtaskToPrio && (
                    <button onClick={() => starSubtaskToPrio(st.text)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2 }} title="Add to today's prio">
                ...
                <div style={{ display: 'flex', gap: 6, marginLeft: 26, marginTop: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                  <input value={(st as any).owner || ''} onChange={e => updateSubOwner(st.id, e.target.value)} placeholder="Owner"
                    style={{ width: 90, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 5, padding: '2px 6px', fontSize: 10, color: '#94a3b8', outline: 'none' }} />
                  <input type="date" value={stDeadline || ''} onChange={e => updateSubDeadline(st.id, e.target.value)}
